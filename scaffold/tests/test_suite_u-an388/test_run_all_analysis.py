...
def test_01_run_cycle_analysis():
    ...
    os.chdir(suite_dir)
    omnium.omnium_main(['omnium', '-D', '-b', 'run', '-t', 'cycle', '--all', 'S0', 'S4'])

def test_02_run_expt_analysis():
    suite_dir = os.getenv('SCAFFOLD_SUITE_UAN388_DIR')
    os.chdir(suite_dir)
    omnium.omnium_main(['omnium', '-D', '-b', 'run', '-t', 'expt', '--all', 'S0', 'S4'])

def test_03_run_suite_analysis():
    suite_dir = os.getenv('SCAFFOLD_SUITE_UAN388_DIR')
    os.chdir(suite_dir)
    omnium.omnium_main(['omnium', '-D', '-b', 'run', '-t', 'suite', '--all', 'S0', 'S4'])
